...
Public-ready exports of reusable Agent Skills, packaged as a native Claude Code plugin with matching specialized agents and usable as standalone Codex skills.
...
- `agents/`: one Claude Code agent per existing skill, each preloading only its matching skill.
...
Install the repository as a native plugin. This loads all five skills and their matching agents without copying folders or depending on the current working directory.
...
Claude Code can also discover these skills from `~/.claude/skills/` or a project's `.claude/skills/` directory. Copy any or all of the existing skill folders there. For example:
...
description: Finds and compares current travel deals
skills:
  - travel-deal-finder
---
```
